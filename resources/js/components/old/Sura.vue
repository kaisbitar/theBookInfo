<template>
  <div>
    ...
    <input type="text" class="searchBox" v-model="search" placeholder="ابحث عن اية"/>
    <div v-if="!loading"  class="suraBlock card" v-on:click="changeToScreen">
      ...
      <div class="versesBlock" @click ="clickToCalculate">
        ...
        <div
          class="verse card"
          v-for="(verse, index) in filteredList"
          v-bind:key="index"
          ref="thisVerse"
        ...
        >
          <div class="SuraLettersCount">
            <span class="verseIndex btn btn-warning">آية {{verse[0]}}</span>
          </div>
          ...
          <div class="verseText container-fluid">
            <span class="btn btn-success">{{verse[1].verseText}}</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    fetchVerses: function() {
      ...
      fetch("api/verses-map/" + this.suraFileName, { method: "GET" })
        ...
        .then(res => {
          this.verses = res;
          this.verses = Object.entries(this.verses);
          this.showSura = true;
          this.loading = false;
        });
    },
  ...
  computed: {
    filteredList() {      
      return this.verses.filter(post => {
        if(post[0]!== "SuraLettersCount"){
          // if(post[1].verseText.includes(this.search)){
          //   post[1].verseText = "<span style='color:red;'>" + post[1].verseText + "</span>";
          //   return post[1].verseText
          // }
          return post[1].verseText.includes(this.search);
        }
      })
    },
    suraName(){
      return this.$attrs.suraName 
    }
  },
  mounted() {},
  created() {},
