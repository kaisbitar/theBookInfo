<template>
  ...
  <div id="theBook" class="card">
    <input type="text" class="searchBox" v-model="search" placeholder="ابحث عن سورةاو اية او كلمة او حرف"/>
      <div class="listWrap">
        <div :class="{ smallSuraItemBlock: isSmallList}" class="suraItemBlock" v-if="showList">
          ...
          <div
            ...
            class="listItems btn"
            v-for="(suraIndexItem, index) in filteredList"
            v-bind:key="index"
...
export default {
    ...
    fetchList() {
      fetch("api/quran-index", { method: "GET" })
        ...
        .then(res => {
          this.surasList = res;
          this.loading = false;
          this.setSuraInPlay(this.suraFileName);
        });
    },
  ...
  computed: {
    filteredList() {
      return this.surasList.filter(post => {
        return post.suraName.includes(this.search)
      })
    }
  },
  mounted() {
